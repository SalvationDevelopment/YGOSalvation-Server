...
export async function PATCH(request, { params }) {
  ...
  const updated = await Cover.findByIdAndUpdate(id, updates, { new: true, runValidators: true });
  if (!updated) {
    return NextResponse.json({ success: false, error: "Not found" }, { status: 404 });
  }

  return NextResponse.json({ success: true, cover: serializeCover(updated) });
}
...
export async function DELETE(request, { params }) {
  ...
  await connectToDatabase();
  const result = await Cover.findByIdAndDelete(id);
  if (!result) {
    return NextResponse.json({ success: false, error: "Not found" }, { status: 404 });
  }
