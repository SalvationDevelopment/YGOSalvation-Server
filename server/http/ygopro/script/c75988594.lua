...
function c75988594.initial_effect(c)
	...
	e2:SetCondition(c75988594.con)
	e2:SetTarget(c75988594.thtg)
	e2:SetOperation(c75988594.thop)
	c:RegisterEffect(e2)
end
function c75988594.con(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetSummonType()==SUMMON_TYPE_SYNCHRO
end
function c75988594.filter(c)
	return c:IsRace(RACE_MACHINE) and c:IsAbleToHand()
end
function c75988594.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c75988594.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c75988594.filter,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectTarget(tp,c75988594.filter,tp,LOCATION_GRAVE,0,1,1,nil)
