...
function c9753964.initial_effect(c)
	--Synchro Summon
	aux.AddSynchroProcedure2(c,nil,aux.NonTuner(c9753964.synfil))
	c:EnableReviveLimit()
	--Negate Effect
	...
	e1:SetTarget(c9753964.negtg)
	e1:SetOperation(c9753964.negop)	
	c:RegisterEffect(e1)
...
function c9753964.negop(e,tp,eg,ep,ev,re,r,rp)
...
function c9753964.spfil(c,e,tp)
...
function c9753964.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c9753964.spfil(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c9753964.spfil,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c9753964.spfil,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
