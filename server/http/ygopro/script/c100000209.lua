...
function c100000209.cfilter(c,tp)
	return c:IsReason(REASON_BATTLE) and c:IsPreviousLocation(LOCATION_MZONE) and c:GetPreviousControler()==tp 
		and c:IsLevelBelow(4) and (c:IsSetCard(0x93) or c:IsSetCard(0x83))
end
...
function c100000209.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local code=nil
	if a:IsReason(REASON_BATTLE) and (a:IsSetCard(0x93) or a:IsSetCard(0x83)) then code=a:GetCode() end
	if t:IsReason(REASON_BATTLE) and (t:IsSetCard(0x93) or t:IsSetCard(0x83)) then code=t:GetCode() end	
