...
function c6945.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	...
	e1:SetCondition(c6945.fscon)
	e1:SetOperation(c6945.fsop)
	c:RegisterEffect(e1)
	--summon success
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_SINGLE)
	e2:SetCode(EFFECT_MATERIAL_CHECK)
	e2:SetValue(c6945.matcheck)
	c:RegisterEffect(e2)
	--Double attack
	...
	e4:SetDescription(aux.Stringid(6945,0))
	e4:SetCategory(CATEGORY_TOHAND)
	e4:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e4:SetCode(EVENT_TO_GRAVE)
	e4:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DELAY)
	e4:SetCondition(c6945.thcon)
	e4:SetTarget(c6945.thtg)
	e4:SetOperation(c6945.thop)
	c:RegisterEffect(e4)
end

function c6945.fscon(e,g,gc,chkf)
	if g==nil then 
		return false 
	end
	if gc then 
...
function c6945.fsop(e,tp,eg,ep,ev,re,r,rp,gc,chkf)
	if gc then 
		...
		mg:AddCard(gc)
		if g1:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(6945,1)) then
...
function c6945.thcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:IsPreviousLocation(LOCATION_MZONE) and bit.band(c:GetSummonType(),SUMMON_TYPE_FUSION)==SUMMON_TYPE_FUSION 
end
function c6945.filter(c)
	return c:IsSetCard(0x9b) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
function c6945.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c6945.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c6945.filter,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectTarget(tp,c6945.filter,tp,LOCATION_GRAVE,0,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,g,1,0,0)
end
function c6945.thop(e,tp,eg,ep,ev,re,r,rp)
