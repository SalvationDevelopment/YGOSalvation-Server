...
function c92510265.initial_effect(c)
	...
	e3:SetCondition(c92510265.thcon)
	e3:SetTarget(c92510265.thtg)
	e3:SetOperation(c92510265.thop)
	c:RegisterEffect(e3)
end

function c92510265.rmfil1(c)
	return c:IsFaceup() and (c:IsAttribute(ATTRIBUTE_LIGHT) or c:IsAttribute(ATTRIBUTE_DARK))
end
function c92510265.rmfil2(c)
	return c:IsAbleToRemove()
end
function c92510265.rmtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return false end
	if chk==0 then return Duel.IsExistingTarget(c92510265.rmfil1,tp,LOCATION_REMOVED,0,1,nil)
		and Duel.IsExistingTarget(c92510265.rmfil2,tp,0,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g1=Duel.SelectTarget(tp,c92510265.rmfil1,tp,LOCATION_REMOVED,0,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_TOGRAVE,g1,1,0,0)
	e:SetLabelObject(g1:GetFirst())
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g2=Duel.SelectTarget(tp,c92510265.rmfil2,tp,0,LOCATION_ONFIELD,1,1,nil)
...
function c92510265.thcon(e,tp,eg,ep,ev,re,r,rp)
...
function c92510265.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c92510265.thfilter(chkc) end
