...
function c1219.initial_effect(c)
	...
	e1:SetTarget(c1219.target)
	e1:SetOperation(c1219.activate)
	c:RegisterEffect(e1)
end
function c1219.dfilter(c)
	return c:IsFaceup() and c:IsDestructable()
end
function c1219.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return false end
	if chk==0 then return Duel.IsExistingTarget(c1219.dfilter,tp,LOCATION_MZONE,0,1,nil)
		and Duel.IsExistingTarget(Card.IsAbleToDeck,tp,0,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g1=Duel.SelectTarget(tp,c1219.dfilter,tp,LOCATION_MZONE,0,1,1,nil)
