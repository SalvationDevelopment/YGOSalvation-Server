...
function c63364266.initial_effect(c)
	...
	e1:SetCondition(c63364266.spcon)
	e1:SetTarget(c63364266.sptg)
	e1:SetOperation(c63364266.spop)
	c:RegisterEffect(e1)
	--atk
	...
	e2:SetValue(c63364266.atkval)
	c:RegisterEffect(e2)
end
function c63364266.spcon(e,tp,eg,ep,ev,re,r,rp)
...
function c63364266.filter(c)
	return c:IsFaceup() and c:IsRace(RACE_WARRIOR) and c:IsAttribute(ATTRIBUTE_EARTH) and c:IsType(TYPE_SYNCHRO)
end
function c63364266.atkval(e,c)
	return Duel.GetMatchingGroupCount(c63364266.filter,c:GetControler(),LOCATION_MZONE,LOCATION_MZONE,c)*300
end
