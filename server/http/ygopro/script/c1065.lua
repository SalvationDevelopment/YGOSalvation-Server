...
function c1065.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(1065,0))
	e1:SetCategory(CATEGORY_COUNTER)
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e1:SetCode(EVENT_SUMMON_SUCCESS)
	e1:SetTarget(c1065.addct)
	e1:SetOperation(c1065.addc)
	c:RegisterEffect(e1)
	local e2=e1:Clone()
	...
	e3:SetValue(c1065.attackup)
	c:RegisterEffect(e3)
	--destroy rep
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringid(1065,1))
	e4:SetCategory(CATEGORY_COUNTER)
	e4:SetType(EFFECT_TYPE_IGNITION)
	e4:SetCountLimit(1)
	e4:SetRange(LOCATION_MZONE)
	e4:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e4:SetTarget(c1065.addct2)
	e4:SetOperation(c1065.addc2)
	c:RegisterEffect(e4)
end
function c1065.addct(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_COUNTER,nil,1,0,0x21)
end
function c1065.addc(e,tp,eg,ep,ev,re,r,rp)
	if e:GetHandler():IsRelateToEffect(e) then
		e:GetHandler():AddCounter(0x21,1)
	end
end
function c1065.attackup(e,c)
	return c:GetCounter(0x21)*300
end
function c1065.addct2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(1065,1))
	Duel.SelectTarget(tp,Card.IsFaceup,tp,LOCATION_ONFIELD,0,1,1,e:GetHandler())
end
function c1065.addc2(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsFaceup() and tc:IsRelateToEffect(e) then
		...
		if tc:GetFlagEffect(1065)~=0 then return end
		local e1=Effect.CreateEffect(c)
		e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_CONTINUOUS)
		e1:SetCode(EFFECT_DESTROY_REPLACE)
		e1:SetTarget(c1065.reptg)
		e1:SetOperation(c1065.repop)
		e1:SetReset(RESET_EVENT+0x1fe0000)
		tc:RegisterEffect(e1)
		tc:RegisterFlagEffect(1065,RESET_EVENT+0x1fe0000,0,0)
	end
end
function c1065.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return not e:GetHandler():IsReason(REASON_RULE) and e:GetHandler():GetCounter(0x21)>0 end
	return true
end
function c1065.repop(e,tp,eg,ep,ev,re,r,rp,chk)
