...
function c23581825.filter1(c,e,tp)
	...
	return c:IsType(TYPE_XYZ) and c:IsSetCard(0xba) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
		and Duel.IsExistingMatchingCard(c23581825.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,c,rk+2,c:GetCode())
end
function c23581825.filter2(c,e,tp,mc,rk,code)
...
function c23581825.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.SelectTarget(tp,c23581825.filter1,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
...
function c23581825.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c23581825.filter2,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,tc,tc:GetRank()+2,tc:GetCode())
	local sc=g:GetFirst()
	if sc then
