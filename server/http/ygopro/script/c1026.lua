...
function c1026.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(1026,1))
	e2:SetCategory(CATEGORY_DRAW)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e2:SetCode(EVENT_TO_GRAVE)
	e2:SetCondition(c1026.drcon)
	e2:SetTarget(c1026.drtg)
	e2:SetOperation(c1026.drop)
	c:RegisterEffect(e2)
end
function c1026.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if ct==1 then 
		...
		local ac=0
		Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(1026,2))
...
function c1026.drcon(e,tp,eg,ep,ev,re,r,rp)
	return bit.band(r,REASON_DESTROY)~=0 and e:GetHandler():GetPreviousControler()==tp
		and e:GetHandler():IsPreviousLocation(LOCATION_ONFIELD)
end
function c1026.drtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetTargetPlayer(tp)
	Duel.SetTargetParam(1)
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,1)
end
function c1026.drop(e,tp,eg,ep,ev,re,r,rp)
