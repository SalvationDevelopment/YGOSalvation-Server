...
function c36857073.initial_effect(c)
	--Synchro Summon
	aux.AddSynchroProcedure2(c,nil,aux.NonTuner(c36857073.synfil))
	c:EnableReviveLimit()
...
function c36857073.sptg1(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c36857073.spfilter1(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>-1 and
		Duel.IsExistingTarget(c36857073.spfilter1,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
...
function c36857073.spcon(e,tp,eg,ep,ev,re,r,rp)
	return ep~=tp
end
function c36857073.spfil1(c,e,tp)
	return c:IsType(TYPE_TUNER) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and Duel.IsExistingMatchingCard(c36857073.spfil2,tp,LOCATION_GRAVE,0,1,nil,e,tp,c:GetLevel())
end
function c36857073.spfil2(c,e,tp,lv)
...
function c36857073.sptg2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>1 and Duel.IsExistingMatchingCard(c36857073.spfil1,tp,LOCATION_DECK,0,1,nil,e,tp) end
...
function c36857073.spop2(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g1=Duel.SelectMatchingCard(tp,c36857073.spfil1,tp,LOCATION_DECK,0,1,1,nil,e,tp)
	if g1:GetCount()>0 then
		local tc=g1:GetFirst()
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local g2=Duel.SelectMatchingCard(tp,c36857073.spfil2,tp,LOCATION_GRAVE,0,1,1,tc,e,tp,tc:GetLevel())
		g1:Merge(g2)
