...
function c94454495.initial_effect(c)
	...
	e3:SetTarget(c94454495.destg)
	e3:SetOperation(c94454495.desop)
	c:RegisterEffect(e3)
	local e4=e3:Clone()
	...
	e2:SetCondition(c94454495.spcon)
	e2:SetCost(c94454495.spcost)
	e2:SetTarget(c94454495.sptg)
	e2:SetOperation(c94454495.spop)
	c:RegisterEffect(e2)
end
...
function c94454495.filter(c,e,tp)
...
function c94454495.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c94454495.filter,tp,LOCATION_DECK,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
function c94454495.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c94454495.filter,tp,LOCATION_DECK,0,1,1,nil,e,tp)
