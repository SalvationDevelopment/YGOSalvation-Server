...
function c99315583.initial_effect(c)
	...
	e2:SetCost(c99315583.thcost)
	e2:SetTarget(c99315583.thtg)
	e2:SetOperation(c99315583.wop)
	c:RegisterEffect(e2)
end
...
function c99315583.thcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToRemoveAsCost() end
	Duel.Remove(e:GetHandler(),POS_FACEUP,REASON_COST)
end
function c99315583.thfilter(c)
	return (c:IsSetCard(0x1373) or (c:IsSetCard(0x1374) and c:IsType(TYPE_SPELL+TYPE_TRAP))) and c:IsAbleToHand()
end
function c99315583.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c99315583.thfilter,tp,LOCATION_GRAVE,0,1,e:GetHandler()) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_GRAVE)
end
function c99315583.wop(e,tp,eg,ep,ev,re,r,rp)
...
function c99315583.thop(e,tp,eg,ep,ev,re,r,rp)
	if not Duel.IsExistingMatchingCard(c99315583.thfilter,tp,LOCATION_GRAVE,0,1,nil) then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c99315583.thfilter,tp,LOCATION_GRAVE,0,1,1,nil)
