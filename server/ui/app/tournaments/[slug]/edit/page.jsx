...
import { fetchLeagues, fetchTournament, updateTournament } from '../../../../util/tournament.api';
...
export default function EditTournamentPage({ params }) {
    ...
    useEffect(() => {
        ...
        Promise.all([
        ...
        ]).then(([leagues, tournament]) => {
            if (!active) {
                return;
            }

            const nextCatalog = leagues.length ? leagues.map((league) => ({
                id: league.slug,
                name: league.name,
            ...
            setFormValues({
                ...
                leagueId: tournament.leagueId || nextCatalog[0].id,
                format: tournament.format || 'Swiss',
                capacity: String(tournament.capacity || 16),
                scheduledStart: toDateTimeInputValue(tournament.startAt),
                configuredRoundCount: String(tournament.rounds || 4),
                checkInRequired: Boolean(tournament.checkInRequired),
                gracePeriodMinutes: String(tournament.graceMinutes || 10),
                ranked: Boolean(tournament.ranked),
                visibility: tournament.visibility || 'public',
                reminderOffsets: tournament.reminderOffsets || ['24h', '4h', '30m'],
            });
            setLoadError('');
        }).catch((error) => {
            if (!active) {
            ...
            setLoadError(error.data?.error || error.message || 'Failed to load tournament edit state.');
        }).finally(() => {
            if (active) {
                setLoading(false);
            }
    ...
    async function onSubmit(event) {
        ...
        if (validationErrors.length || !tournamentId) {
            return;
        }

        setSubmitting(true);
        try {
            const updated = await updateTournament(tournamentId, formValues);
            setSubmitMessage(`Tournament updated: ${updated.name}`);
        } catch (error) {
            setSubmitMessage(error.data?.error || error.message || 'Tournament update failed.');
        } finally {
            setSubmitting(false);
        }
    }
    ...
    if (loading) {
        return (
            <div id='tournamentcreatepage' className='tournamentpanel'>
                ...
                <h2>Loading Edit Form</h2>
                <p className='tournamentlede'>Loading tournament and league data.</p>
            </div>
        );
    }
    ...
    if (loadError) {
        return (
            <div id='tournamentcreatepage' className='tournamentpanel'>
                <p className='tournamenteyebrow'>Tournament Hosting</p>
                <h2>Edit Unavailable</h2>
                <p className='tournamentlede'>{loadError}</p>
                <Link className='tournamentcta' href={`/tournaments/${slug}`}>Back To Tournament</Link>
            </div>
        );
    }
    ...
    return (
        <div id='tournamentcreatepage' className='tournamentpanel'>
            ...
            <p className='tournamentlede'>
                Owner-only before-start edit flow backed by tournament persistence.
            </p>
            ...
            <div className='tournamentcalendargrid tournamentcreategrid'>
                <form className='tournamentpanel tournamentcreateform' onSubmit={onSubmit}>
                    <div className='tournamentsectionheader'>
                        <h3>Configuration</h3>
                        <p>Update the event shell, timing, league ruleset, and reminder policy.</p>
                    </div>

                    <div className='tournamentfiltergrid tournamentcreatefields'>
                        <label className='tournamentfield tournamentfieldfull'>
                            <span>Tournament Name</span>
                            <input id='name' type='text' value={formValues.name} onChange={onChange} />
                        </label>

                        <label className='tournamentfield tournamentfieldfull'>
                            <span>Description</span>
                            <textarea id='description' value={formValues.description} onChange={onChange} rows='4' />
                        </label>
                    ...
                    <div className='tournamentsectionheader'>
                        <h3>Reminders</h3>
                        <p>Choose the reminder offsets to keep on the tournament.</p>
                    </div>
                    ...
                    <div className='tournamentformactions'>
                        ...
                        <Link className='tournamentsecondarycta' href={`/tournaments/${slug}`}>Back To Tournament</Link>
                    </div>
                </form>
                ...
                <aside className='tournamentpanel tournamentdaydetail'>
                    <div className='tournamentsectionheader'>
                        <h3>Preview</h3>
                        <p>Live summary of the edited configuration.</p>
                    </div>

                    <div className='tournamentstack'>
                        <article className='tournamenteventcard'>
                            <div className='tournamenteventmeta'>
                                <span className='tournamentstatus' data-status='registration-open'>Edit Preview</span>
