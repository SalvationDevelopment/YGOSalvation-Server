...
class Field {
    ...
    reIndex(player, location) {
        ...
        var zone = filterlocation(filterPlayer(this.stack, player), location);
 ...
 * @returns {Pile[]} a stack of cards that are in only one index
 */
 ...
 * @param {Pile[]} deck a items The array containing the items This function is in no way optimized.
 * @returns {undefined}
 */
...
class Game {
    ...
    constructor(callback) {
        ...
        function findUIDCollectionPrevious(uid) {
            return getByUID(previousStack, uid);
        }
        ...
        function makeNewCard(currentLocation, currentController, currentSequence, position, code, index) {
            stack.push(new Pile(currentLocation, currentController, currentSequence, stack.length, code));
        ...
        function startDuel(player1, player2, manual, settings) {
            ...
            player1.main.forEach(function (card, index) {
                stack.push(new Pile('DECK', 0, index, stack.length, card));
            });
            player2.main.forEach(function (card, index) {
                stack.push(new Pile('DECK', 1, index, stack.length, card));
            });

            player1.extra.forEach(function (card, index) {
                stack.push(new Pile('EXTRA', 0, index, stack.length, card));
            });
            player2.extra.forEach(function (card, index) {
                stack.push(new Pile('EXTRA', 1, index, stack.length, card));
            });
            if (manual) {
         ...
         * @returns {Pile[]} colllection of cards
         */
        ...
        function retrylastQuestion() {
            ...
            question(this.lastQuestion.slot, this.lastQuestion.type, this.lastQuestion.options, this.lastQuestion.answerLength, this.lastQuestion.onAnswerFromUser);
        }
