function validateDeck (deck, banlist, database) {
  ...
  for (var card in main) {
    if (main[card] > 3 || side[card] && main[card] + side[card] > 3) {
      validate.error = true;
      validate.msg = "You can't have " + cardAmount+ " copies of " + card;
      return validate;
    }
  }
  ...
  for (var card in banlist.bannedCards) {
    ...
    if (cardAmount > banlist.bannedCards[card]) {
      validate.error = true;
      validate.msg = "You have " + cardAmount+ " copies of " + card + " but the legal amount is " + banlist.bannedCards[card] + " in this ruleset";
      return validate;
    }
  }
  return validate;
